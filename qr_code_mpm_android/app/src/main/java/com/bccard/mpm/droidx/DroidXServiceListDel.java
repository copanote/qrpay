...
public class DroidXServiceListDel extends Activity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.sample_activity_malware_list);
		...
		arradapter = new DroidXServiceListAdapter(this, R.layout.sample_activity_malware_row_list, mVList);
	...
	private void setData(Intent mIntent) {
		...
		tvScanResult.setText(Html.fromHtml(getString(R.string.sample_txt_scanresult_scan_number)+"<b>"+iTotalSize+"</b>&nbsp;&nbsp;"+getString(R.string.sample_txt_scanresult_detected_number)+"<font color='red'><b>"+mVList.size()+"</b></font>"));
	}
	...
	public void onClick(View v) {
		...
		case R.id.btn_scanresult_remove_file:
			...
			if(filelist.size() + applist.size() < 1 ) {
				Toast.makeText(this, R.string.sample_msg_scanresult_need_to_select, Toast.LENGTH_SHORT).show();
				break;
			}
			
			if(filelist.size() > 0) {
				if(!removeFiles(filelist)) {
					Toast.makeText(this, R.string.sample_msg_scanresult_delete_wrong, Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(this, R.string.sample_msg_scanresult_complete_del, Toast.LENGTH_SHORT).show();
					if(applist.size() == 0) {
						malwareCheck();
					}
				}
	...
	public void malwareCheck() {
		...
		String msg = getString(R.string.sample_msg_scanresult_remain_malware).replace("%@1", Integer.toString(iMalwareCount));
